...
#code with chatgpt: replaced manual weighted voting with collections.Counter (inverse-distance weighted)
def majority_vote(neighbours, y_train):
    votes = Counter()
    for dist_, i in neighbours:
        weight = 1.0 / (dist_ + 1e-9)
        votes[y_train[i]] += weight
    return votes.most_common(1)[0][0]

#code with chatgpt: replaced manual weighted KNN with sklearn's KNeighborsClassifier(weights="distance")
def knn_predict(X_train, y_train, X_test, k=3,dist_metric="euclidean", sorting="merge"):
    ...
    knn = KNeighborsClassifier(n_neighbors=k, metric=metric, p=3, weights="distance")
